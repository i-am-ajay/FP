...
public class FPGrowth {
    ...
    public FPGrowth(File file) throws FileNotFoundException {
    ...
    /* choose one pattern at a time from the pattern map. Convert it into tokens, find token frequency 
     * in map. If frquency is more than threshhold. add it to pattern list.
    */
    ...
    // process file values, create list of frequent occuring items and list of items not that frequent.
    private void preProcessing(File file, Map<String, Integer> itemsMaptoFrequencies, Scanner input, List<String> sortedItemsbyFrequencies, Vector<String> itemstoRemove) throws FileNotFoundException {
        // input is scanner having file as input 
        while (input.hasNext()) {
    ...
    private void construct_fpTree(File file, Map<String, Integer> itemsMaptoFrequencies, Scanner input, List<String> sortedItemsbyFrequencies, Vector<String> itemstoRemove) throws FileNotFoundException {
        ...
        for (String itemsforTable : sortedItemsbyFrequencies){
            headerTable.add(new FPtree(itemsforTable)); // contain items with most frequent item, serve as poiters.
        }
        //FPTree constructing
        ...
        // find frquent items for a record and add these to a list in sorted order.
        while (input.hasNextLine()){
            String line = input.nextLine();
            StringTokenizer tokenizer = new StringTokenizer(line);
            // vector for each line where tokens are arranged on the basis of their frequent occurence.
            Vector<String> transactionSortedbyFrequencies = new Vector<String>();
            // arrange frequent occuring token in sorted order.
            while (tokenizer.hasMoreTokens()){
                String item = tokenizer.nextToken();
                ...
                for (String vectorString : transactionSortedbyFrequencies){
                    ...
                    // arragne item in the local list on the basis of how frequent they occur. Find overall
                    //Use frequent item map to find which item occurn more frequent and arrange in the list on the basis
                    // of occurrence frequency.
                    // item more frquent then selected
    ...
    // provide tree and headerTable to the method. Traverse elements from the vector and add to the tree.
    void insert(Vector<String> transactionSortedbyFrequencies, FPtree fptree, Vector<FPtree> headerTable) {
        ...
        //System.out.println("Fp Tree "+fptree.item);
    ...
    private void fpgrowth(FPtree fptree, int threshold, Vector<FPtree> headerTable) {
        ...
        // pass fptree, headerTable(trees with starting nodes most frequent itmes)
        FPgrowth(fptree, null, threshold, headerTable, frequentPatterns);
        int i = 0;
    }
    //find current pattern in the tree and store them in map with their support value and find conditional pattern.
    void FPgrowth(FPtree fptree, String base, int threshold, Vector<FPtree> headerTable, Map<String, Integer> frequentPatterns) {
        // get trees from header table.
        // for each tree in headerTable.
        for (FPtree iteminTree : headerTable){
            ...
            // create a map for conditional pattern nodes.
            ...
            while (iteminTree.next != null){
                ...
                // add conditonal pattern and last node count to the map.
                if (conditionalPattern != null) {
                    conditionalPatternBase.put(conditionalPattern, iteminTree.count);
                }
            }
            // add current pattern and support to the frequent patterns map.
            ...
            /* Get condtioanl patterns in a set, read one pattern at a time from set, convert conditional 
            *  pattern into tokens,put these token to conditionalItemsMap and thier occurence count as value
            *  of map.*/
            ...
            /* Get those values from the conditionalItemMap which has occurence more than threshhold value.
             * Create a trees using these values and tree to conditional_headerTable.*/
            Vector<FPtree> conditional_headerTable = new Vector<FPtree>();
            // read each item from the map.
            for (String itemsforTable : conditionalItemsMaptoFrequencies.keySet()) {
            // if the count of the map is greter that threashhold.create a tree and add this tree to a vector.   
            ...
            // call conditional_fptree_constructor.
    ...
    private void print() throws FileNotFoundException{
        ...
        // specify the name of the file to which output should be written.
        //Formatter output = new Formatter("G:\\projects\\meenakshi ma'am\\files\\output.text");
        Formatter output = new Formatter("files\\output.text");
        int count=0;
        for (String frequentPattern : frequentPatterns.keySet()) {
            //System.out.println(++count+":"+frequentPattern);
